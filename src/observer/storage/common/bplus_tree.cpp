...
RC BplusTreeHandler::create(const char *file_name, std::vector<const FieldMeta *> field_metas) {
  ...
  file_header->order =
          ((int) BP_PAGE_DATA_SIZE - sizeof(IndexFileHeader) - sizeof(IndexNode)) / (key_length + 2 * sizeof(RID));
...
int CompAttrs(AttrType attr_type[], int attr_length[], int len, const char *p1, const char *p2){
  for(int i = 0; i < len; i++){
    int result = CompareKey(p1, p2, attr_type[i], attr_length[i]);
    if(result != 0){
      return result;
    }
    p1 += attr_length[i];
...
RC BplusTreeHandler::find_leaf(const char *pkey, PageNum *leaf_page) {
    ...
    while (0 == node->is_leaf) {
        for (i = 0; i < node->key_num; i++) {
            tmp = CmpKey(file_header_.attr_type, file_header_.attr_length,file_header_.attr_num, pkey,
...
RC BplusTreeHandler::get_entry(const char *pkey, RID *rid) {
    ...
    for (i = 0; i < leaf->key_num; i++) {
        const char *p1 = key;
        const char *p2 = leaf->keys + (i * file_header_.key_length);
        if (CompAttrs(file_header_.attr_type, file_header_.attr_length,file_header_.attr_num, p1, p2) ==
            0) {
...
RC BplusTreeHandler::delete_entry(const char *data, const RID *rid) {
    ...
    memcpy(pkey, data, total_length);
    memcpy(pkey + total_length, rid, sizeof(*rid));
...
RC BplusTreeHandler::find_first_index_satisfied(CompOp compop, const char *key, PageNum *page_num, int *rididx) {
    ...
    if (compop==NOT_IN)
      compop = NOT_EQUAL;
    if (compop == LESS_THAN || compop == LESS_EQUAL || compop == NOT_EQUAL || compop == NOT_IN) {
    ...
    while (next > 0) {
        ...
        for (i = 0; i < node->key_num; i++) {
//          char *p1 = const_cast<char *>(pdata);
          const char *p1 = node->keys + i * file_header_.key_length;
          const char *p2 = key;
          tmp = CompAttrs(file_header_.attr_type, file_header_.attr_length, file_header_.attr_num, p1, p2);
//            tmp = CompareKey(node->keys + i * file_header_.key_length, key, file_header_.attr_type,
//                             file_header_.attr_length);
            if (compop == EQUAL_TO || compop == GREAT_EQUAL || compop == EQUAL_IN) {
                if (tmp >= 0) {
...
RC BplusTreeScanner::open(CompOp comp_op, const char *value) {
    ...
    comp_op_ = comp_op;
//    char *value_copy =(char *)malloc(index_handler_.file_header_.key_length - sizeof(RID));
    int total_length = 0;
    for (int i = 0; i < index_handler_.file_header_.attr_num; ++i) {
      total_length += index_handler_.file_header_.attr_length[i];
    }
    char *value_copy = (char *) malloc(total_length);
    if (value_copy == nullptr) {
        LOG_ERROR("Failed to alloc memory for value. size=%d", index_handler_.file_header_.attr_length);
        return RC::NOMEM;
    }
    memcpy(value_copy, value, total_length);
...
bool BplusTreeScanner::satisfy_condition(const char *pkey) {
    ...
    char *p1 = const_cast<char *>(pkey);
    char *p2 = const_cast<char *>(value_);
    IndexFileHeader file_header = index_handler_.file_header_;
    int res = CompAttrs(file_header.attr_type , file_header.attr_length , file_header.attr_num
            ,p1 ,p2);
    bool flag = false;
