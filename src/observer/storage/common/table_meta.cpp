...
RC TableMeta::init_sys_fields() {
  ...
  RC rc = field_meta.init(Trx::trx_field_name(), Trx::trx_field_type(), 0, Trx::trx_field_len(), false, true,0, false);
  if (rc != RC::SUCCESS) {
  ...
  rc = nullsmap_meta.init("nullsmap",CHARS,4,4, false, true,0, false);
    if (rc != RC::SUCCESS) {
        LOG_ERROR("Failed to init trx field. rc = %d:%s", rc, strrc(rc));
        return rc;
    }
