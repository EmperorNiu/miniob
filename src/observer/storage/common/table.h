...
public:
  ...
  RC create_index(Trx *trx, const char *index_name, char *attribute_names[], int attr_num, int isUnique);
  const int get_long_fd() const;
...
private:
  ...
  IndexScanner *find_index_for_scan_multi(const ConditionFilter *filter);
  IndexScanner *find_index_for_scan(const DefaultConditionFilter &filter);
  IndexScanner *find_index_for_scan2(const ConditionFilter *filter);
